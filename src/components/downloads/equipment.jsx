...
import { useLazyGetAllEquipmentsQuery } from "@/redux/services/groups";
import { download, generateCsv, mkConfig } from "export-to-csv";
import { format } from "date-fns";
const csvConfig = mkConfig({
  filename: "equipments_export",
...
export default function Equipment() {
  ...
  const [getAllEquipments, { isLoading: isLoadingEquipments }] =
    ...
    <div>
      <Formik
        ...
        onSubmit={async (values, { resetForm }) => {
          ...
          const result = await getAllEquipments(filterObject);
          // Extract the data array
          const dataForCSV = result?.data?.data || [];
          const flatDataForCSV = dataForCSV?.map((item) => ({
            equipment_name: item.equipment_name,
            next_modernization_date: item.next_modernization_date,
            created_date: item.created_date,
            model_number: item.model_number,
            last_modernization_parts: item.last_modernization_parts,
            year_of_installation: item.year_of_installation,
            equipment_type: item.equipment_type,
            es_pulse_equipment_number: item.es_pulse_equipment_number,
            oem_service_equipment_number: item.oem_service_equipment_number,
            brand_name: item.brand_name,
            site_name: item.site_name,
            region: item.region,
            // Flatten specification
            controller_model: item.specification?.controller_model,
            suspension_type: item.specification?.suspension_type,
            door_controller_brand: item.specification?.door_controller_brand,
            door_controller_model: item.specification?.door_controller_model,
            in_car_media_brand: item.specification?.in_car_media_brand,
            length: item.specification?.length,
            function_type: item.specification?.function_type,
            design_type: item.specification?.design_type,
            speed: item.specification?.speed,
            max_load: item.specification?.max_load,
            in_car_media_model: item.specification?.in_car_media_model,
            floor_from: item.specification?.floor_from,
            floor_to: item.specification?.floor_to,
            stops: item.specification?.stops,
            travel: item.specification?.travel,
            width: item.specification?.width,
            height: item.specification?.height,
            drive_model: item.specification?.drive_model,
